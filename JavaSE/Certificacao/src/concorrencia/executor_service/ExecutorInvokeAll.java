...
public class ExecutorInvokeAll {
	...
	public void runApp() throws InterruptedException, ExecutionException {
		...
		for (Future<AtomicInteger> future : resultFuture) {
			// Didn't deliberately put a timeout here for the get method. Remember, the
			// invoke All does not return until the task is done.
			System.out.println("Status of future : " + future.isDone() + ". Result of future : " + future.get().get());
		}

		executorService.shutdown();

		// You might as well call a resultFuture.get(0).get().get() and that would give
		// you the same
		// result since all your worker threads hold reference to the same atomicinteger
		// sum.
